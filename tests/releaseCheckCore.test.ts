...
describe("releaseCheckCore", () => {
  ...
  it("fails when public alpha decision docs are missing", () => {
    ...
    const withoutPdfDiagnosticsReport = requiredDocs.filter((file: string) => file !== "docs/PHASE_3C_PDF_TRANSLATION_FAILURE_DIAGNOSTICS_REPORT.md");
    const withoutLayoutAwarePdfReport = requiredDocs.filter((file: string) => file !== "docs/PHASE_3D_LAYOUT_AWARE_PDF_EXTRACTION_REPORT.md");
    ...
    expect(check(withoutPdfDiagnosticsReport, {}).failures.some((failure: string) => failure.includes("PHASE_3C_PDF_TRANSLATION_FAILURE_DIAGNOSTICS_REPORT.md"))).toBe(true);
    expect(check(withoutLayoutAwarePdfReport, {}).failures.some((failure: string) => failure.includes("PHASE_3D_LAYOUT_AWARE_PDF_EXTRACTION_REPORT.md"))).toBe(true);
  });
...
function defaultContent(file: string): string {
  if (file === "package.json") {
    return JSON.stringify({ version: "0.3.3-alpha.0" });
  }
  if (file === "package-lock.json") {
    return JSON.stringify({ version: "0.3.3-alpha.0", packages: { "": { version: "0.3.3-alpha.0" } } });
  }
  if (file === "README.md") {
    return "# Readme\n\nAlpha warning\n\nWindows unsigned warning\n\nv0.3.3-layout-aware-pdf-extraction\n";
  }
  if (file === "CHANGELOG.md") {
    return "# v0.3.3-layout-aware-pdf-extraction\n";
  }
